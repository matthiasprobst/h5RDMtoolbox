...
def dump(filename, format):
    ...
    if fmt in ('ttl', 'turtle'):
        ...
        # pass through; rdflib will raise on unsupported
        fmt = fmt
...
def server(filename, host, port, no_structural, no_contextual, file_uri):
    ...
    # Import here to avoid adding fastapi/uvicorn requirement at import-time
    from h5rdmtoolbox.server import run_server
    run_server(host=host, port=port, filename=str(filename), structural=structural, contextual=contextual, file_uri=file_uri)
