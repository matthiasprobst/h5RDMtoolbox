...
from h5rdmtoolbox.conventions.identifier import StandardizedNameTable, CFStandardNameTable

fluid_standard_name_table = StandardizedNameTable('Fluid_Standard_Name',
                                                  table_dict={
...
piv_name_table_dict = fluid_standard_name_table._dict.copy()
...
piv_standard_name_table = StandardizedNameTable(name='PIV_Standard_Name', table_dict=piv_name_table_dict,
                                                version_number=1, contact='matthias.probst@kit.edu',
                                                institution='Karlsruhe Institute of Technology',
                                                valid_characters='[^a-zA-Z0-9_]')
...
if __name__ == '__main__':
    ...
    fluid_standard_name_table.to_xml(
        pathlib.Path(__file__).parent / 'snxml' / f'fluid-v{fluid_standard_name_table.version_number}.xml',
        parents=True)
    piv_standard_name_table.to_xml(
        pathlib.Path(__file__).parent / 'snxml' / f'piv-v{piv_standard_name_table.version_number}.xml')

    # fluid_convention = NameIdentifierConvention(fluid_name_table, name='Fluid_Standard_Name',
    ...
    #                             parents=True)
    # piv_convention = NameIdentifierConvention(piv_name_table, name='PIV_Standard_Name',
    #                                           version=1, contact='matthias.probst@kit.edu',
    #                                           institution='Karlsruhe Institute of Technology')
    # _ = piv_convention.to_xml(pathlib.Path(__file__).parent / 'snxml' / f'piv-v{piv_convention.version}.xml')
    #
    cf_convention_table = fetch_cf_standard_name_table()
