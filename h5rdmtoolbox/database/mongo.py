...
def type2mongo(value: Any) -> Any:
    ...
    try:
        ...
        warnings.warn(
            f"Could not determine/convert {value}. Try to continue with type {type(value)} of {value}. "
            f"Original error: {e}"
        )
    return str(value)
...
def _check_file_exists(
    filename: Union[str, pathlib.Path], record_id: Optional[Any] = None
) -> Optional[pathlib.Path]:
    """Check if file exists and return Path object or None.

    Parameters
    ----------
    filename : str or pathlib.Path
        Filename from database record.
    record_id : Any, optional
        Record identifier for error messages.

    Returns
    -------
    pathlib.Path or None
        Validated file path, or None if file doesn't exist.
    """
    filepath = pathlib.Path(filename)
    if not filepath.exists():
        msg = f"File '{filepath}' referenced in database"
        if record_id:
            msg += f" (record: {record_id})"
        msg += " no longer exists, skipping."
...
def _generate_dataset_document(
        ...
        for i in range(dataset.shape[axis]):
            ...
            if dims is not None:
                for dim in dims:
                    if not isinstance(dim, h5py.Dataset):
                        raise TypeError(
                            f"Dimension must be of type h5py.Dataset, not {type(dim)}"
                        )
                    dim_ls.append(dim)
            ...
            if len(dataset.dims[axis]) > 0:
                for iscale in range(len(dataset.dims[axis])):
                    ...
                    if dim.ndim != 1:
                        warnings.warn(
                            f"Dimension scale dataset must be 1D, not {dim.ndim}D. Skipping"
                        )
                    else:
    ...
    raise NotImplementedError(
        "This method is under heavy construction. Currently, "
...
def _insert_group(
    ...
    if not flatten_tree:
        tree = group.get_tree_structure(recursive=recursive, ignore_attrs=ignore_attrs)
...
class MongoDB(ExtHDF5DBInterface):
    ...
    def find_one(self, *args, **kwargs) -> Optional[lazy.LHDFObject]:
        ...
        dataset or group is returned. Returns None if the file no longer exists."""
        res = self.collection.find_one(*args, **kwargs)
        if res:
            if "filename" not in res:
                raise KeyError(
                    "No filename in result. This could mean, that the "
            ...
            filepath = _check_file_exists(res["filename"], res.get("_id"))
            if filepath is None:
                return None
    ...
    def find(self, *args, **kwargs) -> Generator[lazy.LHDFObject, None, None]:
        ...
        Records with missing files are skipped with a warning.
