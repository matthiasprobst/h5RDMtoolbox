...
""",
        ...
        with h5tbx.use("h5tbx"):
            with h5tbx.File() as h5:
                ds = h5.create_dataset(
                    "ds", data=np.arange(10), units="m/s", attrs={"comment": "test"}
                )
    ...
    def test_create_datasets_from_csv(self):
        ...
        csv_filename1 = h5tbx.utils.generate_temporary_filename(
            suffix=".csv", touch=False
        )
        self.assertEqual(csv_filename1.suffix, ".csv")
        self.assertFalse(csv_filename1.exists())
        csv_filename1 = h5tbx.utils.generate_temporary_filename(
            suffix=".csv", touch=True
        )
        self.assertEqual(csv_filename1.suffix, ".csv")
        ...
        with h5tbx.File() as h5:
            with self.assertRaises(ValueError):
                h5.create_datasets_from_csv(
                    csv_filenames=csv_filename1, combine_opt="invlaid"
                )
            h5.create_datasets_from_csv(csv_filenames=csv_filename1)
            self.assertEqual(
                pathlib.Path(h5["x"].attrs["source_filename"]).resolve().absolute(),
                csv_filename1.resolve().absolute(),
            )
            self.assertEqual(
                h5["x"].attrs["source_filename_hash_md5"],
                h5tbx.utils.get_checksum(csv_filename1),
            )
            self.assertEqual(
                pathlib.Path(h5["y"].attrs["source_filename"]).resolve().absolute(),
                csv_filename1.resolve().absolute(),
            )
            self.assertEqual(
                h5["y"].attrs["source_filename_hash_md5"],
                h5tbx.utils.get_checksum(csv_filename1),
            )
        ...
        with h5tbx.File() as h5:
            x, y = h5.create_dataset_from_csv(
                csv_filename=csv_filename1, shape=(2, 2), dimension=None
            )
        ...
        with h5tbx.File() as h5:
            h5.create_datasets_from_csv(
                csv_filenames=[csv_filename1, csv_filename2], combine_opt="concatenate"
            )
            self.assertEqual(h5["x"].shape, (8,))
            filenames = sorted(
                [
                    pathlib.Path(f).resolve().absolute()
                    for f in h5["y"].attrs["source_filename"]
                ]
            )
            ref_filenames = sorted(
                [csv_filename1.resolve().absolute(), csv_filename2.resolve().absolute()]
            )
            ...
            self.assertEqual(
                ...
                [
                    h5tbx.utils.get_checksum(csv_filename1),
                    h5tbx.utils.get_checksum(csv_filename2),
                ],
            )
        ...
        with h5tbx.File() as h5:
            h5.create_datasets_from_csv(
                csv_filenames=[csv_filename1, csv_filename2],
                combine_opt="stack",
                axis=0,
            )
    ...
    def test_slicing(self):
        with h5tbx.File() as h5:
            ds_scale = h5.create_dataset(
                "time", data=np.linspace(0, 1, 10), make_scale=True, overwrite=True
            )
            ds = h5.create_dataset(
                "grp/data",
                shape=(10, 20, 30),
            ...
            self.assertEqual(
                list(ds[1:4, 1:4, :].coords.keys()),
                [
                    "time",
                ],
            )
            self.assertEqual(
                list(ds[1:4, 1:4, 1:4].coords.keys()),
                [
                    "time",
                ],
            )
    ...
    def test_coord_selection(self):
        with h5tbx.File() as h5:
            ...
            time = h5.create_dataset(
                "time", data=np.linspace(0, 1, 30), make_scale=True
            )
            ds = h5.create_dataset(
                "data", shape=(10, 20, 30), attach_scales=(x, y, time)
            )
    ...
    def test_conditional_slicing(self):
        with h5tbx.File() as h5:
            ...
            ds = h5.create_dataset(
                "data", np.random.rand(100, 200, 100), attach_scale=("time", "y", "x")
            )
            ...
            np.testing.assert_equal(
                h5.data[h5.data.time == 66, :, :],
                h5.data.values[66, :, :].reshape(1, 200, 100),
            )
    ...
    def test_Group(self):
        with h5tbx.File() as h5:
            ...
            self.assertEqual(
                list(grp.get_datasets("data")),
                [
                    grp["data"],
                ],
            )
            self.assertEqual(
                list(grp.get_datasets("dat*")),
                [
                    grp["data"],
                ],
            )
            self.assertEqual(
                sorted(grp.get_datasets(".*")),
                [
                    grp["data"],
                ],
            )
            ...
            h5.grp["New"] = (
                ...
                dict(attrs=dict(name="my_dataset")),
            )
            ...
            self.assertEqual(
                str(h5.grp),
                '<HDF5 wrapper group "/grp" (members: 2, convention: "h5py")>',
            )
            ...
            h5.create_string_dataset(
                "str2", ("a", "b", "c", "dddd"), overwrite=True, attrs={"a": "b"}
            )
            self.assertTrue(h5["str2"].name, "/str2")
    ...
    def test_create_img_dataset(self):
        ...
        with h5tbx.File() as h5:
            ...
            for i in range(5):
                np.testing.assert_equal(
                    ds.values[i, ...], np.ones(shape=(20, 10)) * (i + 1) * 10
                )
            # reset imgreader
            imgreader._index = 0
        ...
        with h5tbx.File() as h5:
            ...
            ds = h5.create_dataset_from_image(arr_list, "testimg", axis=0)
            self.assertEqual(ds.shape, (5, 20, 10))
            self.assertEqual(ds.chunks, (1, 20, 10))
            for i in range(5):
                np.testing.assert_array_almost_equal(
                    ds.values[i, ...], arr_list[i], decimal=5
                )

        imgreader._index = 0
        h5tbx.use("h5py")
        with h5tbx.File() as h5:
            ds = h5.create_dataset_from_image(
                imgreader, "testimg", axis=0, attrs=dict(units="", long_name="test")
            )
            self.assertEqual(ds.shape, (5, 20, 10))
            self.assertEqual(ds.chunks, (1, 20, 10))
            # reset imgreader
            imgreader._index = 0
            ds = h5.create_dataset_from_image(
                imgreader, "testimg2", axis=-1, attrs=dict(units="", long_name="test")
            )
            self.assertEqual(ds.shape, (20, 10, 5))
            self.assertEqual(ds.chunks, (20, 10, 1))
    ...
    def test_special_attribute_types(self):
        with h5tbx.File() as h5:
            ...
            for obj in (h5, ds, grp):
                self.assertTrue(
                    isinstance(obj.attrs, h5tbx.wrapper.h5attr.WrapperAttributeManager)
                )
                ...
                obj.attrs["attribute_of_links_to_ds"] = {
                    "ds": ds,
                    "grp": grp,
                    "astr": "test",
                    "afloat": 3.1,
                ...
                self.assertIsInstance(
                    obj.attrs["attribute_of_links_to_ds"]["astr"], str
                )
                self.assertIsInstance(
                    obj.attrs["attribute_of_links_to_ds"]["afloat"], float
                )
                ...
                for test_val in test_vals:
                    ...
                    self.assertEqual(
                        f"{obj.attrs['mean_with_unit'].to_pint()}", "1.2 m"
                    )
                    self.assertEqual(h5tbx.get_config("ureg_format"), "C~")
                    self.assertEqual(h5tbx.get_ureg().formatter.default_format, "C~")
                    self.assertEqual(
                        h5tbx.get_ureg().formatter.default_format,
                        h5tbx.get_config("ureg_format"),
                    )
                    self.assertEqual(
                        f"{obj.attrs['mean_with_unit'].to_pint()}", "1.2 m"
                    )
                    ...
                    self.assertEqual(
                        obj.attrs["mean_with_unit"], str(h5tbx.get_ureg()(test_val))
                    )
    ...
    def test_compression(self):
        with h5tbx.set_config(hdf_compression="gzip", hdf_compression_opts=5) as _:
            with h5tbx.File() as h5:
                ...
                self.assertEqual(
                    h5tbx.get_config("hdf_compression"),
                    h5["no_compression"].compression,
                )
                self.assertEqual(
                    h5tbx.get_config("hdf_compression_opts"),
                    h5["no_compression"].compression_opts,
                )

                h5.create_dataset(
                    "gzip", data=[1, 2, 3], compression="gzip", compression_opts=1
                )
                self.assertEqual("gzip", h5["gzip"].compression)
                self.assertEqual(1, h5["gzip"].compression_opts)

                h5.create_dataset(
                    "lzf", data=[1, 2, 3], compression="lzf", compression_opts=None
                )
                ...
                with self.assertRaises(ValueError):
                    h5.create_dataset(
                        "lzf2", data=[1, 2, 3], compression="lzf", compression_opts=2
                    )
    ...
    def test_create_dataset_from_xr(self):
        with h5tbx.File() as h5:
            ...
            h5.create_dataset(
                "ds0", data=xr.DataArray(0), compression="gzip", compression_opts=1
            )  # will be ignored
            ...
            h5.create_dataset(
                "ds_coordinate",
                data=xr.DataArray([1, 2, 3], dims=["ds0"], coords={"z": 0}),
            )
            ...
            with self.assertRaises(ValueError):
                h5.create_dataset(
                    "ds",
                    data=xr.DataArray(
                        [1, 2, 3], dims=["time"], coords={"time": [1, 4, 3]}
                    ),
                )
            self.assertEqual(
                h5["ds"],
                h5.create_dataset(
                    "ds",
                    data=xr.DataArray(
                        [1, 2, 3], dims=["time"], coords={"time": [1, 2, 3]}
                    ),
                    overwrite=False,
                ),
            )
            h5.create_dataset(
                "ds",
                data=xr.DataArray(
                    [1, 2, 30], dims=["time"], coords={"time": [1, 2, 3]}
                ),
                overwrite=True,
                dtype="int64",
            )
    ...
    def test_isel_sel(self):
        with h5tbx.File() as h5:
            ...
            h5.create_dataset("vel", data=[1.5, 2.5, 3.5], attach_scales="time")
            np.testing.assert_equal(
                h5["vel"].values[0:2], h5["vel"].isel(time=slice(0, 2)).values
            )
    ...
    def test_isel_multiple_coords_per_axis(self):
        with h5tbx.File() as h5:
            ...
            h5.create_dataset(
                "vel", data=[1.5, 2.5, 3.5], attach_scales=(("time", "another_time"),)
            )
    ...
    def test_unit_conversion_interface(self):
        with h5tbx.File() as h5:
            h5.create_dataset(
                "time", data=[1, 2, 3], attrs=dict(units="s"), make_scale=True
            )
            h5.create_dataset(
                "vel",
                data=[1.5, 2.5, 3.5],
    ...
    def test_multi_isel_and_sel(self):
        with h5tbx.File() as h5:
            ...
            h5.create_dataset(
                "data", shape=(10, 5, 7), attach_scales=("time", "y", "x")
            )
            np.testing.assert_equal(
                h5["data"][0, 0, [0, 2]], h5["data"].values[0, 0, [0, 2]]
            )
            np.testing.assert_equal(
                h5["data"].isel(time=0, y=0, x=[0, 2]), h5["data"].values[0, 0, [0, 2]]
            )

        filename = h5.hdf_filename

        with h5tbx.File(filename) as h5:
            d = h5["data"].sel(x=[4.3, 10.9], time=0.2, method="nearest")
            np.testing.assert_equal(d.x.data, [5, 10])

            d = h5["data"].sel(
                x=np.linspace(4.3, 10.9, 100), time=0.2, method="nearest"
            )
    ...
    def test_create_dataset_with_ancillary_ds(self):
        with h5tbx.File() as h5:
            ...
            h5.create_dataset(
                "flag2", data=[0, 0], dtype="int8", make_scale="the_flag_2"
            )
            ...
            with self.assertRaises(ValueError):
                # wrong shape
                h5.create_dataset(
                    name="vel2", data=[1.5, 2.5, 4.5], attach_scale="flag2"
                )
    ...
    def test_time(self):
        ...
        with h5tbx.File() as h5:
            ...
            h5.create_time_dataset(
                "time3", data=tdata_np, time_format="iso", attrs={"time_format": "ISO"}
            )
            tds3 = h5["time3"][()]
    ...
    def test_timedataset(self):
        ...
        with h5tbx.File() as h5:
            ds = h5.create_time_dataset(
                "time", data=abs_time, time_format="%Y-%m-%dT%H:%M:%S.%f"
            )
    ...
    def test_time_as_coord(self):
        ...
        with h5tbx.File() as h5:
            ...
            self.assertEqual(
                parse(str(v.time1[0].data).strip("0")), parse(str(t1[0]).strip("0"))
            )
            self.assertEqual(
                parse(str(v.time2[0].data).strip("0")), parse(str(t2[0]).strip("0"))
            )
    ...
    def test_object_data(self):
        ...
        ds = h5tbx.database.find_one(
            h5.hdf_filename, flt={"$basename": "varlen_strings"}
        )
