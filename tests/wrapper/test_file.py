...
class TestFile(unittest.TestCase):
    ...
    def test_attrs1(self):
        with File(mode="w") as h5:
            ...
            self.assertDictEqual(
                dset.attrs["a dict"],
                {"key1": "value1", "key2": 1239.2, "subdict": {"subkey": 99}},
            )
    ...
    def test_attrs2(self):
        ...
        with File(mode="w") as h5:
            ...
            dset = h5.create_dataset(
                "ds", data=1, attrs={"a1": 1, "a2": "str", "a3": {"a": 2}}
            )
            self.assertEqual(dset.attrs.get("a1"), 1)
            self.assertEqual(dset.attrs.get("a2"), "str")
    ...
    def test_tree_structure(self):
        with File() as h5:
            ...
            grp.create_dataset("grp_ds", shape=(2, 40, 3))
            sub_grp = grp.create_group(
                "sub_grp", attrs={"description": "sub group description"}
            )
    ...
    def test_scale_manipulation(self):
        with File(mode="w") as h5:
            ...
            h5.create_dataset(
                "time", data=np.random.rand(10), attrs=dict(long_name="time", units="s")
            )
            h5.create_dataset(
                "temp",
                data=np.random.rand(10),
                attrs=dict(long_name="temperature", units="K"),
        ...
        with h5tbx.use("h5tbx") as cv5:
            with File(mode="w") as h5:
                ...
                ds_scale = h5.create_dataset(
                    "ds_scale", data=1, attrs=dict(units="m/s/V")
                )
    ...
    def test_get_group_names(self):
        with h5tbx.set_config(auto_create_h5tbx_version=False):
            with File(mode="w") as h5:
                ...
                self.assertEqual(
                    h5["one"].get_group_names(), ["three", "three/four", "two"]
                )
                self.assertEqual(
                    sorted(h5["one"].get_group_names(recursive=False)),
                    sorted(["two", "three"]),
                )
                self.assertEqual(
                    sorted(h5["/"].get_group_names(recursive=False)),
                    sorted(
                        [
                            "one",
                        ]
    ...
    def test_get_dataset_names(self):
        with File(mode="w") as h5:
            h5.create_dataset(
                "one", data=1, attrs=dict(long_name="long name", units="")
            )
            h5.create_dataset(
                "two", data=1, attrs=dict(long_name="long name", units="")
            )
            h5.create_dataset(
                "grp/three", data=1, attrs=dict(long_name="long name", units="")
            )
            h5.create_dataset(
                "grp/two", data=1, attrs=dict(long_name="long name", units="")
            )
            self.assertEqual(
                h5.get_dataset_names(), ["grp/three", "grp/two", "one", "two"]
            )
    ...
    def test_init_with_file_object(self):
        ...
        This test ensures that the File class properly initializes when passed
        a file-like object (with read/seek methods). Regression test for the
        double __init__ bug fix.
        """
        with File(mode="w") as h5:
            h5.create_dataset("data", data=[1, 2, 3])
            h5.attrs["test_attr"] = "value"
            original_filename = h5.hdf_filename

        with open(original_filename, "rb") as f:
            file_obj = io.BytesIO(f.read())

        with File(file_obj, mode="r") as h5_reopened:
            self.assertEqual(list(h5_reopened["data"][:]), [1, 2, 3])
            self.assertEqual(h5_reopened.attrs["test_attr"], "value")
