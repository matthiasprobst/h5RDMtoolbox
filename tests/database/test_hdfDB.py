...
class TestHDFDB(unittest.TestCase):
    ...
    def test_find_in_files(self):
        ...
        with h5tbx.File() as h52:
            ...
            h52.create_dataset(
                "dataset", data=np.array([3, 2, 1]), attrs=dict(units="mm/s")
            )

        res = list(
            h5tbx.database.find(
                [h51.hdf_filename, h52.hdf_filename], {"name": "root group"}
            )
        )
        ...
        res = list(
            h5tbx.database.find(
                [h51.hdf_filename, h52.hdf_filename],
                {"name": "grp name"},
                recursive=False,
            )
        )
        ...
        res = h5tbx.database.find_one(
            [h51.hdf_filename, h52.hdf_filename], {"name": "grp name"}, recursive=True
        )
    ...
    def test_dtype_char(self):
        ...
        res = hdfdb.FileDB(h5.hdf_filename).find(
            {"$dtype": {"$regex": "S*"}}, objfilter="dataset", recursive=False
        )
        self.assertIsInstance(res, list)
        self.assertIsInstance(res[0], h5tbx.database.lazy.LDataset)
    ...
    def test_find_shape(self):
        with h5tbx.File(mode="w") as h5:
            ...
            self.assertListEqual(
                sorted([r.name for r in res]), sorted([ds_random.name, ds_half.name])
            )
    ...
    def test_find_one(self):
        with h5tbx.set_config(auto_create_h5tbx_version=False):
            with h5py.File(
                h5tbx.utils.generate_temporary_filename(suffix=".hdf"), "w"
            ) as h5:
                ...
                res_is_dataset = grpdb.find_one(
                    {"$dtype": {"$exists": True}}, recursive=False
                )
                ...
                res = dsdb.find_one(
                    {"$dtype": np.dtype("<f8")}, objfilter="dataset"
                )  # is float64
                self.assertEqual(res, ds)
                res = dsdb.find_one(
                    {"$dtype": np.dtype("<f4")}, objfilter="dataset"
                )  # is float64
                ...
                single_res = gdb_root.find_one(
                    {"long_name": {"$exists": True}}, recursive=True
                )
                self.assertIsInstance(single_res, database.lazy.LGroup)
                self.assertTrue(single_res.basename in ("grp", ""))

                single_res = gdb_root.find_one(
                    {"long_name": {"$exists": False}}, recursive=True
                )
    ...
    def test_find_dict_attr(self):
        with h5tbx.File(mode="w") as h5:
            ...
            self.assertListEqual(
                sorted([r.name for r in res]), sorted([grp.name, ds.name])
            )
    ...
    def test_filesDB_from_folder(self):
        ...
        self.assertListEqual(
            sorted(fdb.filenames), sorted([filename1, filename2, filename3])
        )

        fdb = hdfdb.FilesDB.from_folder(
            folder, hdf_suffixes=[".h5", ".hdf"], recursive=True
        )
        self.assertEqual(len(fdb.filenames), 6)
        self.assertListEqual(
            sorted(fdb.filenames),
    ...
    def test_find_rdf(self):
        ...
        res = h5tbx.database.rdf_find(
            h5.hdf_filename, rdf_predicate="https://schema.org/name"
        )
        print(list(res))
        ...
        res = h5tbx.database.rdf_find(
            h5.hdf_filename, rdf_subject="https://example.org/Matthias"
        )
        res_list = sorted(list(res), key=lambda x: x.name)
        self.assertEqual(len(res_list), 1)
        self.assertEqual(res_list[0].name, "/contact")
        ...
        res = h5tbx.database.rdf_find(
            h5.hdf_filename, rdf_subject="https://example.org/John"
        )
        res_list = sorted(list(res), key=lambda x: x.name)
        self.assertEqual(len(res_list), 1)
    ...
    def test_dtype_kind(self):
        with h5tbx.File() as h5:
            ...
            numerical_datasets = h5.find(
                {"$dtype": lambda x: x.kind in ("f", "i", "u")}
            )
    ...
    def test_custom_find(self):
        with h5tbx.File() as h5:
            h5.create_dataset(
                "u", data=4.3, attrs={"units": "m/s", "standard_name": "x_velocity"}
            )
            h5.create_dataset(
                "v", data=4.3, attrs={"units": "m/s", "standard_name": "v_velocity"}
            )  # invalid sn
            h5.create_dataset(
                "w", data=4.3, attrs={"units": "invalid", "standard_name": "z_velocity"}
            )
            ...
            res = h5.find(
                {
                    "standard_name": lambda x: _valid_standard_name(
                        x, ["x_velocity", "y_velocity", "z_velocity"]
                    )
                }
            )
            self.assertEqual(len(res), 2)
