...
try:
    client = pymongo.MongoClient(serverSelectionTimeoutMS=1.0)
    client.server_info()
...
class TestH5Mongo(unittest.TestCase):
    ...
    def test_make_dict_mongo_compatible(self):
        ...
        self.assertEqual(make_dict_mongo_compatible({"a": 4.1}), {"a": 4.1})
        self.assertEqual(make_dict_mongo_compatible({"a": 4}), {"a": 4})
        with self.assertWarns(UserWarning):
            self.assertEqual(
                make_dict_mongo_compatible({"a": np.array(5.5)}), {"a": np.array(5.5)}
            )
    ...
    def test_insert(self):
        ...
        self.assertIsInstance(
            mongoDBInterface.find_one({"basename": "dataset"}), LDataset
        )
        self.assertIsInstance(
            mongoDBInterface.find({"basename": "dataset"}), types.GeneratorType
        )
    ...
    def test_find_one_with_missing_file(self):
        ...
        mongoDBInterface.collection.insert_one(
            {
                "filename": "/non/existent/file.h5",
                "name": "/dataset",
                "basename": "dataset",
                "hdfobj": "dataset",
            }
        ...
        with self.assertWarns(UserWarning):
            result = mongoDBInterface.find_one({"basename": "dataset"})
        self.assertIsNone(result)

    def test_find_with_missing_file(self):
        """Test that find() skips records with missing files."""
        mongoDBInterface = MongoDB(collection=self.collection)
        with h5py.File("test.h5", "w") as h5:
            h5.create_dataset("dataset", data=[1, 2, 3])
            mongoDBInterface.insert_dataset(h5["dataset"], axis=None)

        record = mongoDBInterface.collection.find_one({"basename": "dataset"})
        record["filename"] = "/non/existent/file.h5"
        mongoDBInterface.collection.find_one_and_replace(
            {"basename": "dataset"}, record
        )

        with self.assertWarns(UserWarning):
            results = list(mongoDBInterface.find({}))
    ...
    def test_sklearn_digits(self):
        ...
        with h5tbx.File(filename, "w") as h5:
            ds_trg = h5.create_dataset("digit", data=digits.target, make_scale=True)
            ds_img = h5.create_dataset("images", shape=(len(digits.images), 8, 8))

            ds_mean = h5.create_dataset(
                "mean", shape=(len(digits.images),), make_scale=True
            )
            ds_diss = h5.create_dataset(
                "dissimilarity", shape=(len(digits.images),), make_scale=True
            )
            ds_corr = h5.create_dataset(
                "correlation", shape=(len(digits.images),), make_scale=True
            )
