...
class TestExtension(unittest.TestCase):
    ...
    def test_onto(self):
        with h5tbx.File() as h5:
            h5.onto.create_person(
                orcid_id="https://orcid.org/0000-0001-8729-0482",
                first_name="Matthias",
                last_name="Probst",
            )
            self.assertEqual(
                h5["Matthias_Probst"].attrs["orcid_id"],
            ...
            self.assertEqual(
                h5["Matthias_Probst"].rdf.subject,
                "https://orcid.org/0000-0001-8729-0482",
            )
    ...
    def test_Magnitude(self):
        with h5tbx.File() as h5:
            h5.create_dataset(
                "x", data=[1, 2, 3], make_scale=True, attrs={"units": "m"}
            )
            h5.create_dataset(
                "y", data=[1, 2, 3, 4], make_scale=True, attrs={"units": "m"}
            )
            h5.create_dataset(
                "u", data=np.random.random((4, 3)), attach_scales=("y", "x")
            )
            h5.create_dataset(
                "v", data=np.random.random((4, 3)), attach_scales=("y", "x")
            )
    ...
    def test_Vector(self):
        ...
        with h5tbx.File() as h5:
            ...
            vec = h5.Vector(u="u", v="v").isel(
                dim_0=slice(None, None, None), dim_1=slice(None, None, None)
            )
            self.assertIsInstance(vec, xr.Dataset)
            self.assertTrue("u" in vec.data_vars)
            self.assertTrue("v" in vec.data_vars)
    ...
    def test_normalize_issue_with_time_vector(self):
        with h5tbx.File() as h5:
            ...
            np.testing.assert_almost_equal(
                uxnorm["x/xref"].values, h5.x.values[()] / 10
            )
            self.assertEqual(uxnorm.units, "m/s")

            uxnorm = (
                u.normalize(ref=10, name="uxnorm")
                .x(xref=10)
                .isel(x=slice(None, None, None))
            )
            np.testing.assert_almost_equal(uxnorm.values, u.values[()] / 10)
            np.testing.assert_almost_equal(
                uxnorm["x/xref"].values, h5.x.values[()] / 10
            ...
            uxnorm = (
                u.normalize(ref=10, name="uxnorm")
                .x(xref=10)
                .sel(x=[1.1, 2.1, 3.1], method="nearest")
            )
            np.testing.assert_almost_equal(uxnorm.values, u.values[()] / 10)
            np.testing.assert_almost_equal(
                uxnorm["x/xref"].values, h5.x.values[()] / 10
    ...
    def test_units_to(self):
        with h5tbx.File(mode="w") as h5:
            ...
            y = h5.create_dataset(
                "y", data=[1, 0, 1], attach_scale="x", attrs={"units": "mm"}
            )
