...
from h5rdmtoolbox.conventions.registration import register_standard_attr, StandardAttribute, register_standard_attribute
...
class TestAccessor(unittest.TestCase):
    ...
    def test_hdf_attribute(self):
        with self.assertRaises(AttributeError):
            @register_standard_attr(Group, name='short_name')
        ...
        register_standard_attribute(shortyname(), cls=Group, overwrite=True)
        ...
        with h5tbx.File() as h5:
            ...
            register_standard_attribute(shortyname(), cls=File, overwrite=True)
            h5.shortyname = 'shorty'
            self.assertIn('shortyname', h5.attrs.keys())

        with self.assertRaises(AttributeError):
            @register_standard_attr(Group)
            class attrs:
                pass
