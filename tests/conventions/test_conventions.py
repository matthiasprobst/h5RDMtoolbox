...
class TestConventions(unittest.TestCase):
    ...
    def test_convention_thread_safety(self):
        """Test that convention state is isolated per thread using ContextVar."""
        import threading
        import time
        ...
        def set_and_get_convention(conv_name):
            h5tbx.use(conv_name)
            barrier.wait()
            results[threading.current_thread().name] = (
                h5tbx.convention.get_current_convention().name
            )
            barrier.wait()
        ...
        t1 = threading.Thread(
            target=set_and_get_convention, args=("h5py",), name="thread1"
        )
        t2 = threading.Thread(
            target=set_and_get_convention, args=("h5tbx",), name="thread2"
        )
        ...
        self.assertEqual(results["thread1"], "h5py")
        self.assertEqual(results["thread2"], "h5tbx")
    ...
    def test_extract_function_info(self):
        from h5rdmtoolbox.convention.generate import (
            extract_function_info,
            validate_specialtype_functions,
        )
        import ast
...
def validate_f1(a, b, c=3, d=2):
    ...
    def test_overwrite_existing_file(self):
        if self.connected:
            ...
            cv = h5tbx.convention.from_repo(
                ZenodoRecord(source=15389242), name="tutorial_convention.yaml"
            )
            ...
            with h5tbx.File(
                ...
                h5.create_dataset(
                    "g1/ds1", data=1, comment="Dataset comment", long_name="dataset 1"
                )
                ...
                self.assertEqual(
                    str(h5.contact), "https://orcid.org/0000-0001-8729-0482"
                )
            ...
            with h5tbx.File(filename) as h5:
                with h5tbx.set_config(ignore_get_std_attr_err=True):
                    with self.assertWarns(
                        h5tbx.errors.StandardAttributeValidationWarning
                    ):
                        h5.data_type
    ...
    def test_add_decoder(self):
        ...
        with self.assertRaises(ValueError):
            h5tbx.register_dataset_decoder(
                multiply_by_2_decoder, decoder_name="multiply_by_2_decoder_v2"
            )
        h5tbx.register_dataset_decoder(
            multiply_by_2_decoder_v2, decoder_name="multiply_by_2_decoder_v2"
        )
        h5tbx.register_dataset_decoder(
            multiply_by_2_decoder_v2,
            decoder_name="multiply_by_2_decoder_v2",
            overwrite=True,
        )
        ...
        # remove decoders
        self.assertTrue(
            "multiply_by_2_decoder" not in cv.remove_decoder("multiply_by_2_decoder")
        )
        self.assertTrue(
            "multiply_by_2_decoder_v2"
            not in cv.remove_decoder("multiply_by_2_decoder_v2")
        )
    ...
    def test_cv_h5tbx(self):
        ...
        self.assertTupleEqual(
            ("scale_and_offset",), h5tbx.wrapper.ds_decoder.decoder_names
        )
        h5tbx.use(None)
        self.assertTupleEqual((), h5tbx.wrapper.ds_decoder.decoder_names)
        h5tbx.use("h5tbx")
        ...
        with h5tbx.File() as h5:
            with self.assertRaises(h5tbx.errors.StandardAttributeError):
                ds = h5.create_dataset(
                    "test", data=1, units="m/s", attrs={"units": "Pa"}
                )
    ...
    def test_convention_file_props(self):
        ...
        with h5tbx.File() as h5:
            ...
            self.assertEqual(
                ...
                sorted(list(h5.standard_attributes)),
            )
            ds = h5.create_dataset("test", data=1, units="m/s")
            self.assertEqual(
                sorted(["units", "symbol"]), sorted(ds.standard_attributes.keys())
            )
    ...
    def test_data_scale_and_offset(self):
        ...
        with h5tbx.File() as h5:
            ...
            self.assertEqual(ds[()].units, "m/s")
            self.assertEqual(float(ds[()].data), 1 + 500.0)
        ...
        with h5tbx.File() as h5:
            ...
            self.assertEqual(
                ds.get_data_scale().attrs["units"], h5tbx.get_ureg().Unit("m/s/V")
            )
            self.assertEqual(str(ds.get_data_offset().attrs["units"]), "km/s")
    ...
    def test_from_zenodo(self):
        if self.connected:
            ...
            self.assertEqual(cv.name, "h5rdmtoolbox-tutorial-convention")
            with h5tbx.File(
                data_type="experimental", contact=h5tbx.__author_orcid__
            ) as h5:
                ...
                h5.create_dataset(
                    "test",
                    data=4.3,
                    standard_name="xx_reynolds_stress",
                    units="m**2/s**2",
                )
                self.assertEqual(str(h5["test"].standard_name), "xx_reynolds_stress")
                self.assertEqual(
                    h5["test"].attrs.raw["standard_name"], "xx_reynolds_stress"
                )
                ...
                self.assertNotEqual(
                    h5.standard_attributes["comment"].description,
    ...
    def test_standard_name_table_as_file(self):
        cv = h5tbx.Convention.from_yaml(
            __this_dir__ / "test_convention.yaml", overwrite=True
        )
        h5tbx.use(cv)
        ...
        with h5tbx.File(
            ...
            standard_name_table=f"file://{snt_file}",
        ) as h5:
    ...
    def test_alternative_attribute(self):
        h5tbx.Convention.from_yaml(
            tutorial.get_convention_yaml_filename(), overwrite=True
        )
    ...
    def test_standard_attributes_and_rdf(self):
        ...
        with h5tbx.use("h5rdmtoolbox-tutorial-convention"):
            with h5tbx.File(
                ...
                self.assertEqual(
                    h5.frdf["data_type"].object, "https://www.wikidata.org/wiki/Q101965"
                )
    ...
    def test_dates(self):
        ...
        with h5tbx.File(list_of_dates=["22.12.2023", "23.12.2023"]) as h5:
            ...
            self.assertEqual(
                h5.list_of_dates, [datetime(2023, 12, 22), datetime(2023, 12, 23)]
            )
        ...
        with self.assertRaises(h5tbx.errors.StandardAttributeError):
            with h5tbx.File(
                specific_date={"date": "22.12.invalid", "dateType": "Created"}
            ) as h5:
                pass
        with self.assertRaises(h5tbx.errors.StandardAttributeError):
            with h5tbx.File(
                specific_date={"date": "22.12.2023", "dateType": "invalid"}
            ) as h5:
                pass
        with h5tbx.File(
            specific_date={"date": "22.12.2023", "dateType": "Created"}
        ) as h5:
    ...
    def test_engmeta_example(self):
        ...
        with h5tbx.File(
            contact=dict(name="Matthias Probst"),
            creator=dict(
        ...
        with self.assertRaises(h5tbx.errors.StandardAttributeError):
            with h5tbx.File(
                contact=dict(invalid="Matthias Probst"),
                creator=dict(
                    name="Matthias Probst",
                    id="https://orcid.org/0000-0001-8729-0482",
                    role="Researcher",
    ...
    def test_read_invalid_attribute(self):
        cv = h5tbx.convention.Convention.from_yaml(
            __this_dir__ / "simple_cv.yaml", overwrite=True
        )
        # h5tbx.use(None)
        # with h5tbx.File() as h5:
        ...
        with h5tbx.use("simple_cv"):
            ...
            with h5tbx.File(creator={"name": "Joe", "invalid": "123"}) as h5:
                self.assertEqual(h5.creator.name, "Joe")
                self.assertEqual(h5.creator.orcid, None)
                with self.assertRaises(AttributeError):
                    self.assertEqual(h5.creator.invalid, "123")
            with self.assertRaises(h5tbx.errors.StandardAttributeError):
                with h5tbx.File(creator={"name": "Joe", "orcid": "123"}) as h5:
                    pass
            with h5tbx.File(
                creator={"name": "Joe", "orcid": h5tbx.__author_orcid__}
            ) as h5:
                self.assertEqual(h5.creator.name, "Joe")
